      ...
      <div className="overflow-hidden rounded text-slate-500 shadow-slate-200">
        <figure className="relative">
          {user.images.filter(img => img.type === "banner").length === 0
            ? <div src="" className="opacity-25 w-full h-64 bg-gray-700"></div>
            : <img src={user.images.filter(img=> img.type === "banner")[0].url} className="opacity-25 w-full h-64"/>}
            <div className="md:flex absolute bottom-0 left-0 w-full p-6 text-white from-slate-900">
              ...
              <div className="md:pl-2 place-content-end">
                {user.player.prefix && (<p className="md:text-xl text-lg text-gray-600 dark:text-gray-400">{user.player.prefix}</p>)}
                {user.player.gamerTag && (<p className="flex md:text-3xl text-2xl dark:text-white">{user.player.gamerTag}</p>)}
                ...
                {user.bio !== null && (<p className="text-base text-gray-700 dark:text-gray-300">{user.bio}</p>)}
              </div>
            </div>
        </figure>
