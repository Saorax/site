...
export function KOEffectStoreView({ koEffects, langs }) {
  ...
  useEffect(() => {
    ...
    else if (isMobile) { setSelectedKOEffect(null); filtersChanged.current = true; }
  }, [
    filterCohort, filterPromo, filterStoreID, storeOnly, filterStoreLabel, filterPromoType,
  ...
  const filtered = useMemo(() => {
    ...
    const arr = data.filter(e => {
      ...
      switch (sortType) {
        ...
        case 'KOEffectIDAsc': return idA - idB;
        case 'KOEffectIDDesc': return idB - idA;
  ...
  const resetFilters = useCallback(() => {
    ...
    if (isMobile) { setSelectedKOEffect(null); filtersChanged.current = true; }
    else { setSelectedKOEffect(filtered[0] || null); }
  }, [filtered, isMobile]);
          ...
          {selectedKOEffect ? (
            <div className="gap-2 flex flex-col">
              <div className="flex flex-col gap-2 pb-3 dark:bg-slate-800 bg-gray-100 p-2 rounded-lg">
                ...
                {Array.isArray(selectedKOEffect.store) && selectedKOEffect.store.flatMap(s => splitTags(s.SearchTags)).filter(Boolean).length > 0 && (
                  <div className="flex flex-col flex-wrap">
                    <span className="text-gray-900 dark:text-white">Store Search Tags</span>
                    <div className="flex flex-wrap gap-2 mt-1">
                      {[...new Set(selectedKOEffect.store.flatMap(s => splitTags(s.SearchTags)).filter(Boolean))].map((tag, idx) => (
                ...
                <div className="flex gap-2 flex-wrap">
                  {selectedKOEffect.promo && (
                    <span className="text-sm px-3 py-1 rounded-lg bg-purple-500 dark:bg-purple-700 text-gray-900 dark:text-white">Promo Type: {selectedKOEffect.promo.Type}</span>
                  )}
                  {selectedKOEffect.bp && (
                    <span className="text-sm px-3 py-1 rounded-lg bg-emerald-500 dark:bg-emerald-700 text-gray-900 dark:text-white">
                      Battle Pass Season {String(selectedKOEffect.bp.ID).replace('BP', '').replace('-', ' ')}{selectedKOEffect.bp.Tier ? ` (Tier ${selectedKOEffect.bp.Tier})` : ''}
                    </span>
                  )}
                  {selectedKOEffect.entitlement && (
                    <span className="text-sm px-3 py-1 rounded-lg bg-amber-500 dark:bg-amber-700 text-gray-900 dark:text-white">
                      {(langs.content?.[selectedKOEffect.entitlement.DisplayNameKey]?.replace('!', '') || selectedKOEffect.entitlement.EntitlementName) + ' DLC'}
                    </span>
                  )}
                  {selectedKOEffect.store?.length > 0 && [...new Set(selectedKOEffect.store.map(sd => sd.Label).filter(Boolean))].map((label, idx) => (
                  ...
                  {selectedKOEffect.store?.length > 0 && [...new Set(selectedKOEffect.store.map(s => s.TimedPromotion ?? '').filter(Boolean))].map((promo, idx) => (
              ...
              <div className="flex flex-col lg:flex-row gap-2">
                <div className="lg:w-1/2 flex flex-col gap-2">
                  ...
                  {getProcessedStoreData(selectedKOEffect).map((sd, idx) => (
                    ...
                    <div className='dark:bg-slate-800 bg-gray-100 p-2 rounded-lg'>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-lg mt-2">
                        <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-lg">
                          ...
                          <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.EntitlementName}</span>
                        </div>
                        {selectedKOEffect.entitlement.EntitlementID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-lg">
                            <span className="font-bold text-gray-600 dark:text-gray-300">Entitlement ID</span>
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.EntitlementID}</span>
                          </div>
                        )}
                        {selectedKOEffect.entitlement.SteamAppID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-lg">
                            <span className="font-bold text-gray-600 dark:text-gray-300">Steam App ID</span>
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.SteamAppID}</span>
                          </div>
                        )}
                        {selectedKOEffect.entitlement.SonyEntitlementID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-2xl">
                            ...
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.SonyEntitlementID}</span>
                          </div>
                        )}
                        {selectedKOEffect.entitlement.SonyProductID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-2xl">
                            <span className="font-bold text-gray-600 dark:text-gray-300">Sony Product ID</span>
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.SonyProductID}</span>
                          </div>
                        )}
                        {selectedKOEffect.entitlement.NintendoConsumableID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-2xl">
                            <span className="font-bold text-gray-600 dark:text-gray-300">Nintendo Consumable ID</span>
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.NintendoConsumableID}</span>
                          </div>
                        )}
                        {selectedKOEffect.entitlement.NintendoEntitlementID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-2xl">
                            <span className="font-bold text-gray-600 dark:text-gray-300">Nintendo Entitlement ID</span>
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.NintendoEntitlementID}</span>
                          </div>
                        )}
                        {selectedKOEffect.entitlement.XB1EntitlementID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-2xl">
                            <span className="font-bold text-gray-600 dark:text-gray-300">XB1 Entitlement ID</span>
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.XB1EntitlementID}</span>
                          </div>
                        )}
                        {selectedKOEffect.entitlement.XB1ProductID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-2xl">
                            <span className="font-bold text-gray-600 dark:text-gray-300">XB1 Product ID</span>
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.XB1ProductID}</span>
                          </div>
                        )}
                        {selectedKOEffect.entitlement.XB1StoreID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-2xl">
                            <span className="font-bold text-gray-600 dark:text-gray-300">XB1 Store ID</span>
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.XB1StoreID}</span>
                          </div>
                        )}
                        {selectedKOEffect.entitlement.AppleEntitlementID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-2xl">
                            <span className="font-bold text-gray-600 dark:text-gray-300">Apple Entitlement ID</span>
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.AppleEntitlementID}</span>
                          </div>
                        )}
                        {selectedKOEffect.entitlement.AndroidEntitlementID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-2xl">
                            <span className="font-bold text-gray-600 dark:text-gray-300">Android Entitlement ID</span>
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.AndroidEntitlementID}</span>
                          </div>
                        )}
                        {selectedKOEffect.entitlement.UbiConnectID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-2xl">
                            <span className="font-bold text-gray-600 dark:text-gray-300">Ubi Connect ID</span>
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.UbiConnectID}</span>
                          </div>
                        )}
                        {selectedKOEffect.entitlement.UbiConnectPackageID && (
                          <div className="flex flex-col bg-gray-100 dark:bg-slate-900 p-2 rounded-2xl">
                            <span className="font-bold text-gray-600 dark:text-gray-300">Ubi Connect Package ID</span>
                            <span className="text-gray-900 dark:text-white">{selectedKOEffect.entitlement.UbiConnectPackageID}</span>
                          </div>
                        )}
                      </div>
                    </div>
